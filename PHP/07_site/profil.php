...
if (!internauteEstConnecte()) {  // si le membre n'est pas connecté il ne doit pas y avoir accès à la page profil
    header('location:connexion.php');  // nous l'invitons à se connecter
    exit();
}

// 2 - prépareation du profil à afficher :
// debug($_SESSION);
extract($_SESSION['membre']);  // extrait tous les indices de l'array sous forme de variables aux quelles on affecte la valeur dans l'array. Exemple : $_SESSION['membre']['pseudo'] devient $pseudo = $_SESSION['membre']['pseudo'];
