...
if (!empty($_POST)) {
    ...
    $photo_bdd = '';  // pardéfaut la photo est vide en BDD

    // debug($_FILES);

    if (!empty($_FILES['photo']['name'])) { // s'il y a un nom de fichier dans la superglobal $_FILES, c'est que je suis en train d'uploader un fichier. L'indice "photo" correspond au "name" du champ dans le formulaire
...
$contenu .= '<div class="bg-success">Le produit à bien été enregistré !</div>';
...
?>
    ...
    <ul class="nav nav-tabs">
        <li><a href="gestion_boutique.php" class="nav-link">Affichage de produits</a></li>
...
<form action="" method="post" enctype="multipart/form-data">  <!-- "multipart/form-data" spécifie que le formulaire envoie des données d'un fichier uploadé et du texte : ici permet d'uploader une photo pour le produit -->

    <input type="hidden" name="id_produit" id="id_produit" value="0"> <!-- ce champ caché est utile pour la modification d'un produit afin de l'identifier dans la requête SQL. La valeur 0 par défaut signifie que le produit n'existe pas en BDD, et qu'on est entrain de le créer  -->
    ...
    <input type="file" name="photo" id="photo"><br><br> <!-- Le type file permet d'uploader un fichier et de remplir la superglobal $_FILES. Ne pas oublier l'attribut enctype dans la balise <form> de ce formulaire -->
